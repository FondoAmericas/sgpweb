	...
	<tbody>
		<c:forEach items="${listBien }" var="bien" varStatus="indice">
			...
			<tr class="<c:out value="${classIdi}"></c:out>">
				...
				<td style="width: 10%; text-align: left;" class="hide">
				<c:choose>
					<c:when test="${bien.cantOrgBienTransferido == 0 }"><label>
				<a
						href="javascript:modificarBien('<c:out value="${bien.bienID }" ></c:out>',
						...
						id="eliminarRegistro" class="linkSelecciona">Modificar</a><br /><a
						href="javascript:eliminarBien('<c:out value="${bien.bienID }" ></c:out>')"
						id="eliminarRegistro" class="linkSelecciona">Eliminar</a></label>	
					</c:when>
					<c:otherwise>
