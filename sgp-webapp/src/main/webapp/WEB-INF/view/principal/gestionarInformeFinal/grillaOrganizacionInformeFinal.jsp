...
<table border="0" width="100%" class="table-clasico">
	...
	<tr class="<c:out value="${classIdi}"></c:out>">
			...
			<td style="width: 20%;text-align: center;" class="hide"><label><a
						href="javascript:modificarOrganizacionInformeFinal('<c:out value="${organizacion.organizacionID}" ></c:out>','<c:out value="${organizacion.nombreOrganizacion }"></c:out>','<c:out value="${organizacion.situacionFinal }"></c:out>')"
						...
						href="javascript:eliminarRegistroInformeFinal('<c:out value="${organizacion.organizacionID}" ></c:out>','organizacion')"
						id="eliminarRegistro" class="linkSelecciona">Eliminar</a> 
						</c:when>
						<c:otherwise>Organizacion en <br/>bien transferido</c:otherwise>
